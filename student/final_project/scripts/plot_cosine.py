...
#
# If your grouping column isn't auto-detected, specify it:
#   python .\plot_cosine.py --baseline base_cosine.csv --pirate pirate_cosine.csv --out_prefix cosine --group_col category
...
def main():
    ...
    ap.add_argument("--baseline", required=True, help="Baseline cosine CSV (must contain cosine_style_score)")
    ap.add_argument("--pirate", required=True, help="Pirate cosine CSV (must contain cosine_style_score)")
    ap.add_argument("--out_prefix", default="cosine", help="Prefix for saved plot files")
    ap.add_argument("--group_col", default=None, help="Column name to group by (optional)")
    ap.add_argument("--score_col", default="cosine_style_score", help="Score column name")
    args = ap.parse_args()

    df_base = pd.read_csv(args.baseline)
    df_pir = pd.read_csv(args.pirate)

    require_col(df_base, args.score_col, "baseline CSV")
    require_col(df_pir, args.score_col, "pirate CSV")
    ...
    if group_col is not None and (group_col not in df_base.columns or group_col not in df_pir.columns):
        print(f"[Warning] group_col='{group_col}' not present in BOTH CSVs. Disabling grouped plot.")
        group_col = None
    ...
    plt.xlabel("Cosine Style Score")
    plt.ylabel("Count")
    plt.title("Cosine Style Score Distribution")
    plt.legend()
    plt.tight_layout()
    ...
    plt.boxplot(
        ...
        labels=["Baseline", "Pirate"],
        showmeans=True
    )
    plt.ylabel("Cosine Style Score")
    plt.title("Cosine Style Score: Baseline vs Pirate")
    plt.tight_layout()
    ...
    # ==========================================
    # Plot 3: Mean cosine score by prompt group
    # ==========================================
    if group_col is not None:
        ...
        # Make x-axis readable (horizontal, spaced)
        plt.xticks(x, labels, rotation=0)
        plt.ylabel("Mean Cosine Style Score")
        plt.title("Mean Cosine Style Score by Prompt Type")
        plt.legend()
        plt.tight_layout()
    ...
    # ==========================================
    # Optional: Print summary stats to console
    # ==========================================
